  ...
  return (
    <>
      <section className="relative overflow-hidden border-b border-line">
        ...
        <div className="relative mx-auto grid max-w-6xl gap-16 px-6 pb-20 pt-20 md:pt-28 lg:grid-cols-[1.15fr_0.85fr] lg:pb-28">
          <div>
            <p className="kicker reveal" translate="no">
              {t("kicker")}
            </p>
            <h1 className="font-display mt-6 text-[clamp(2.2rem,6vw,4.25rem)] leading-[1.05] text-paper reveal">
              {t("headline")}
            </h1>
            <p className="mt-8 max-w-xl text-lg leading-8 text-paper-soft reveal">{t("lede")}</p>
      ...
      <section className="border-b border-line">
        <div className="mx-auto grid max-w-6xl gap-10 px-6 py-16 md:grid-cols-2 md:py-24">
          <h2 className="font-display text-4xl leading-tight md:text-5xl">{t("statementB")}</h2>
          <p className="text-lg leading-8 text-paper-soft">{t("body")}</p>
        </div>
      </section>
        ...
        <div className="mt-10 grid gap-px bg-line md:grid-cols-3">
          {[
            ...
            <article key={card.href} className="bg-ink p-8 md:p-10">
              <h3 className="font-display text-3xl" translate="no">
                {card.name}
              </h3>
              <p className="mt-4 min-h-[4.5rem] text-sm leading-7 text-paper-soft">{card.lede}</p>
              {card.codes && (
                <p className="mt-6 font-mono text-[11px] tracking-[0.16em] text-bronze" translate="no">
                  {plusCodes.join("  ·  ")}
                </p>
              )}
              <Link
                href={card.href}
                className="mt-8 inline-block font-mono text-[11px] uppercase tracking-[0.16em] text-bronze hover:text-bronze-2"
              >
                {card.cta}
              </Link>
            </article>
      ...
      <section className="border-b border-line">
        <div className="mx-auto max-w-6xl px-6 py-16 md:py-24">
          ...
          <p className="mt-6 max-w-3xl text-lg leading-8 text-paper-soft">{t("trustStrip")}</p>
          <ButtonLink href="/trust" variant="ghost" className="mt-8">
            {nav("trust")}
          </ButtonLink>
        </div>
      </section>
