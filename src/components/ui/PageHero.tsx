  ...
  return (
    <header className="relative overflow-hidden border-b border-line">
      ...
      <div className="relative mx-auto max-w-6xl px-6 pb-16 pt-28 md:pb-24 md:pt-36">
        ...
        <h1 className="font-display mt-6 max-w-4xl text-[2.35rem] leading-[1.08] text-paper md:text-6xl reveal" style={{ animationDelay: "80ms" }}>
          {title}
        </h1>
        {lede && (
          <p className="mt-7 max-w-2xl text-[1.05rem] leading-8 text-paper-soft reveal" style={{ animationDelay: "160ms" }}>
            {lede}
          </p>
        )}
        {children && <div className="mt-10 reveal" style={{ animationDelay: "240ms" }}>{children}</div>}
      </div>
    </header>
  );
}
    ...
    <ol className="divide-y divide-line border-y border-line">
      {items.map((item, i) => (
        <li key={item} className="grid grid-cols-[4.5rem_1fr] items-baseline gap-4 py-5 md:grid-cols-[6rem_1fr]">
          <span className="font-mono text-[11px] tracking-[0.18em] text-bronze">
